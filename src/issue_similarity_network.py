...
Outputs: issue_edges.csv, communities_issue.csv,
centrality_issue.csv, and GML visualization.
"""
...
def company_issue_edges(df, max_issue_df=MAX_ISSUE_DF, min_weight=MIN_WEIGHT):
    ...
    df = df.groupby(["fortune_name", "issue_code"],
                    as_index=False)["amount_allocated"].sum()
    ...
    if max_issue_df is not None:
        df_for_matrix = filter_bills_by_prevalence(df, max_issue_df, unit_col="issue_code")

    # Build firms × issue codes pivot matrix
    pivot = df_for_matrix.pivot_table(
        index="fortune_name", columns="issue_code",
...
def main():
    ...
    edges = company_issue_edges(df, min_weight=MIN_WEIGHT)

    print(f"Issues: {df['issue_code'].nunique()}  |  "
    ...
    comm_df = (
        ...
        .sort_values(["community_issue", "fortune_name"])
        .reset_index(drop=True)
    )
    comm_df.to_csv(DATA_DIR / "communities_issue.csv", index=False)
    print(f"\nCommunity assignments -> {DATA_DIR / 'communities_issue.csv'}")
    ...
    if WRITE_GML or CENTRALITY_K > 0:
        ...
        if CENTRALITY_K > 0:
            ...
            print(f"\nCentrality -> {DATA_DIR / 'centrality_issue.csv'}")
    ...
    if TOP_K > 0:
        ...
        plot_circular(H,
                      title=f"Top {TOP_K} Fortune 500 Issue Similarity Network",
                      path=PNG_PATH)
