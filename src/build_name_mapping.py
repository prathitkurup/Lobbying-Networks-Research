"""
Prathit Kurup
Enrich fortune500_name_mapping.json with subsidiary names sourced from
SEC Exhibit 21 filings (10-K annual reports). Only subsidiaries that
appear as actual client names in the LobbyView clients table are added.

Steps:
  1. Fetch SEC company tickers → fuzzy-match each Fortune 500 company
  2. Pull most recent 10-K → find and parse Exhibit 21
  3. Cross-reference parsed subsidiaries against LobbyView client names
  4. Write matched subsidiaries into the JSON mapping

Run: python build_name_mapping.py
