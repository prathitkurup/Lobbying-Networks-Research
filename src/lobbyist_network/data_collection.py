...
DATA_PATH = "/Users/vfigueroa/Library/CloudStorage/OneDrive-BowdoinCollege/Desktop/Prathit/Lobbying-Networks-Research/src/lobbyist_network"
OUT_PAIRS = "/Users/vfigueroa/Library/CloudStorage/OneDrive-BowdoinCollege/Desktop/Prathit/Lobbying-Networks-Research/src/lobbyist_network/company_lobbyingfirm_pairs_116_2019.csv"
...
# --- light normalization (no suffix stripping) ---
...
# Precompute normalized patterns once
fortune_patterns = []
for canonical, variations in fortune_500_names.items():
    canon_norm = norm_basic(canonical)
    vars_norm = [norm_basic(v) for v in variations]
    fortune_patterns.append((canonical, canon_norm, vars_norm))

def map_to_fortune(client_name: str):
    cn = norm_basic(client_name)
    if not cn:
        return None
    for canonical, canon_norm, vars_norm in fortune_patterns:
        if canon_norm and (canon_norm in cn or cn in canon_norm):
            return canonical
        for v in vars_norm:
            if v and (v in cn or cn in v):
                return canonical
    return None

def to_bool(x) -> bool:
    # robust for True/False, 0/1, t/f, yes/no
    s = str(x).strip().lower()
    return s in {"true", "t", "1", "yes", "y"}

def main():
    clients = pd.read_csv(f"{DATA_PATH}/lobbyview_clients.csv", dtype=str)
    reports  = pd.read_csv(f"{DATA_PATH}/lobbyview_reports.csv", dtype=str)

    # If your client name column is not exactly "client_name", uncomment:
    # print("clients cols:", clients.columns.tolist())
    # print("reports cols:", reports.columns.tolist())

    # join reports -> client_name using lob_id (client id) 
    df = reports.merge(
    ...
    # Filter: year + external (K-street) only  [oai_citation:4‡lobbyview_reports_codebook.pdf](sediment://file_0000000054ec71f5be02118542d81e7b)
    df["filing_year"] = pd.to_numeric(df["filing_year"], errors="coerce")
    df["is_self_filer"] = df["is_self_filer"].apply(to_bool)

    df = df[(df["filing_year"] == TARGET_YEAR) & (~df["is_self_filer"])].copy()

    # Map client -> Fortune canonical
    df["company"] = df["client_name"].apply(map_to_fortune)
    df = df[df["company"].notna()].copy()

    # Company–Firm pairs
    pairs = (
        df[["company", "registrant_id", "registrant_name"]]
        .dropna(subset=["registrant_id"])
        .drop_duplicates()
    ...
    print("Wrote:", OUT_PAIRS, "rows:", len(pairs))
    print("Unique companies:", pairs["company"].nunique())
    print("Unique lobbying firms:", pairs["registrant_id"].nunique())
